...
(function ($) {
  ...
  // 12. Close Handlers //
  $(
    ".tp-offcanvas-close-btn, .tp-offcanvas-2-close-btn, .tp-search-close-btn, .body-overlay, .tp-offcanvas-menu nav ul li a",
  ).on("click", function () {
    $(
  ...
  function moving_text() {
    if ($(".moving-text").length > 0) {
      gsap.utils.toArray(".moving-text").forEach((section, index) => {
        ...
        const [x, xEnd] = index % 2 ? [distance, 0] : [0, distance];
  ...
  function handleDataSpeedAttr() {
    const elements = document.querySelectorAll(
      "[data-speed], [data-speed-original]",
    );
    elements.forEach((el) => {
